...
(function () {
    ...
    function getVarId(v) {
        try { return v?.id ?? (v.getId ? v.getId() : null); } catch (e) { return null; }
    }

    function getVarName(v) {
        try { return v?.name ?? (v.getName ? v.getName() : null); } catch (e) { return null; }
    }

    function getVarType(v) {
        try { return v?.type ?? (v.getType ? v.getType() : "Global"); } catch (e) { return "Global"; }
    }
    ...
    function renameWorkspaceVariable(ws, varObj, newName) {
        try {
            ...
            console.warn("[ExtVars] renameWorkspaceVariable error:", e);
        }
        return false;
    }
    ...
    function reorderVariablesInWorkspace(ws, category, orderedList) {
        ...
        try {
            const snapshot = orderedList.map(v => ({ id: v.id, name: v.name, type: v.type }));
    ...
    function countVariableUsage(ws, varDef) {
        ...
        for (const block of allBlocks) {
            ...
            try {
                ...
                console.warn("[ExtVars] Variable count check error:", e);
            }
        }
...
.ev-row { display: flex; justify-content: space-between; align-items: center; padding: 8px; background: #171717; border-radius: 6px; margin-bottom: 8px; cursor: grab; }
.ev-row:active { cursor: grabbing; }
...
.ev-title { font-weight: 700; font-size: 24px; }
            ...
            for (const v of arr) {
                ...
                row.addEventListener("mousedown", (e) => {
                    ...
                    function onMouseMove(ev) {
                        ...
                        row.style.left = ev.clientX - row.offsetWidth / 2 + "px";
                        row.style.top = ev.clientY - row.offsetHeight / 2 + "px";
                    }
                ...
                editBtn.onclick = () => {
                    const newName = prompt("New variable name:", v.name);
                    if (!newName) return;
    ...
    if (plugin?.registerCommand) {
        plugin.registerCommand({
            name: "Open Extended Variable Manager",
            id: "extvar_manager_open",
            callback: openModal
        });
    } else {
        console.warn("[ExtVars] Plugin command registration unavailable.");
    }
