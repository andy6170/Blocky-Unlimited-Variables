// BF6 Extended Variable Manager - fixed and fully working
(function () {
    ...
    function deleteWorkspaceVariable(ws, idOrName) {
        try {
            const map = workspaceGetVariableMap(ws);
            if (!map) return false;
            if (map.deleteVariableById) { map.deleteVariableById(idOrName); return true; }
            if (map.deleteVariable) { map.deleteVariable(idOrName); return true; }
            if (map.removeVariable) { map.removeVariable(idOrName); return true; }
            if (map.getVariables && Array.isArray(map.getVariables())) {
    ...
    function renameWorkspaceVariable(ws, varObj, newName) {
        try {
            ...
            if (!found && map.getVariable) found = map.getVariable(id) || map.getVariable(getVarName(varObj));
    ...
    function openAdd() {
        ...
        if (!name) return;
        const type = prompt("Enter variable type/category:", "Global") || "Global";
        const id = makeNextSequentialIdFromWorkspace();
        createWorkspaceVariable(ws, name, type, id);
        openModal();
    }
    ...
    function blockUsesVariable(block, varId, seen = new Set()) {
        if (!block || !varId || seen.has(block.id)) return false;
        seen.add(block.id);

        if (block.type && block.fields && block.fields.VAR) {
            if (block.fields.VAR.id === varId) return true;
        }
        ...
        if (block.inputs) {
            for (const key in block.inputs) {
                ...
                if (blockUsesVariable(inputBlock, varId, seen)) return true;
            }
        }
    ...
    function countVariableUsage(ws, varDef) {
        ...
        for (const block of allBlocks) {
            if (blockUsesVariable(block, varDef.id)) count++;
        }
        return count;
    }
    ...
    function openModal() {
        ...
        if (ws) {
            ...
            for (const cat of CATEGORIES) {
                for (const v of live[cat] || []) {
                    usageCounts[v.id] = 0;
                    for (const block of allBlocks) {
                        if (blockUsesVariable(block, v.id)) usageCounts[v.id]++;
                    }
                }
            }
        }
        ...
        function rebuildList() {
            ...
            const addBtn = document.createElement("button"); addBtn.className = "ev-btn ev-add"; addBtn.innerText = "Add"; addBtn.onclick = () => openAdd(); header.appendChild(addBtn);
            ...
            for (const v of arr) {
                ...
                const delBtn = document.createElement("button"); delBtn.className = "ev-btn ev-del"; delBtn.innerText = "Delete"; delBtn.onclick = () => {
                    ...
                    try { deleteWorkspaceVariable(ws, v.id) || deleteWorkspaceVariable(ws, v.name); } catch(e){ console.warn(e); }
